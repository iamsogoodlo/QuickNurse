...
import PatientNurseMap from '../../../components/maps/PatientNurseMap'; // Update the path to the correct location
...
const FindNursesPanel: React.FC = () => {
  ...
  const [modalServiceType, setModalServiceType] = useState<ServiceType>(ServiceType.GENERAL);
  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    const response = await findNearbyNurses(
      position.lat,
      position.lng,
      radius, // Now using number instead of parsing string
      specialty,
      serviceType,
      token
    );
  ...
  const handleNurseRequest = (nurse: NearbyNurse) => {
    setSelectedNurse(nurse);
    setModalServiceType(serviceType);
    setShowModal(true);
  };

  const handleOrderPlaced = (nurse: NearbyNurse) => {
    setOrderPlacedWith(nurse);
    setShowModal(false);
    setSelectedNurse(null);
  };

  const serviceTypeOptions = [
    { value: ServiceType.GENERAL, label: 'General Checkup' },
    { value: ServiceType.FEVER_CHECK, label: 'Fever Check' },
    { value: ServiceType.BLOOD_PRESSURE, label: 'Blood Pressure Monitoring' },
    { value: ServiceType.WOUND_CARE, label: 'Wound Care' },
    { value: ServiceType.MEDICATION_HELP, label: 'Medication Help' },
    { value: ServiceType.HEALTH_CONSULTATION, label: 'Health Consultation' },
    { value: ServiceType.DIABETES_CARE, label: 'Diabetes Care' },
    { value: ServiceType.IV_THERAPY, label: 'IV Therapy' },
  ];
        ...
        <div className="lg:w-1/3 space-y-4 bg-gray-50 p-4 rounded-lg shadow">
           ...
           <form onSubmit={handleSearch} className="space-y-3">
            ...
            <Select
              ...
              label="Service Type"
              value={serviceType}
              onChange={(e: ChangeEvent<HTMLSelectElement>) => setServiceType(e.target.value as ServiceType)}
        ...
        <div className="lg:w-2/3">
          ...
          {!isLoading && nurses.length > 0 && (
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-x-6 gap-y-8">
                {nurses.map(nurse => (
                  <NurseCard
                    key={nurse.nurse_id}
                    nurse={nurse}
                    onRequest={handleNurseRequest}
                  />
                ))}
              </div>
            </div>
