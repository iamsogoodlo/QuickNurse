...
const RequestServiceModal: React.FC<Props> = ({ 
  isOpen, 
  onClose, 
  defaultServiceType, 
  nurse,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Ensure coordinates exist, otherwise use fallback or show error
    if (!patient.address.coordinates) {
      setMessage('Missing location coordinates');
      return;
    }
    ...
    const payload = {
      ...
      location: {
        type: 'Point' as const,
        coordinates: patient.address.coordinates as [number, number], // Type assertion since we validated above
        address: {
